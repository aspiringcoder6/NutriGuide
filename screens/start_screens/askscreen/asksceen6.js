...
export default function AskScreen6({ navigation }) {
  const user = auth.currentUser;
  const [change, setChange] = useState(false);
  const resetScreen = () => {
    navigation.dispatch(StackActions.replace("Askscreen5"));
  };
  ...
  const handleNext = async () => {
    if (answer == "") {
      setPlaceholder("Please Input Weight");
      ...
      await updateDoc(doc(db, "users", user.uid), {
        desireWeight: parseInt(answer),
      });
      navigation.navigate("Askscreen7");
    }
  };
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.header}>Your desired Weight:</Text>
